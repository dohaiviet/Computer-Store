...
<html>
    <head>
        ...
        <link rel="icon" type="logo/png" sizes="32x32" href="./images/logo1.png">
        <style>
            .box-content{
                margin: 0 auto;
                width: 800px;
        ...
        if (isset($_GET['action']) && $_GET['action'] == 'edit') {
            ...
            if (!empty($user)) {
                ...
                <div id="edit_user" class="box-content">
                    <h1>Sửa trạng thái đơn hàng "<?= $user['id'] ?>"</h1>
                    <form action="./edit_donhang.php?action=edit" method="Post" autocomplete="off">
                        <input type="hidden" name="id" value="<?= $user['id'] ?>" />
                        <select name="status" class="btn2">
                            <option value="0" <?= (int)$user['status'] == 1 ? 'disabled':'' ?> <?= (int)$user['status'] == 2 ? 'disabled':'' ?> <?= (int)$user['status'] == 3 ? 'disabled':'' ?> <?= (int)$user['status'] == 0 ? 'selected':'' ?>>Đang xử lý</option>
                            <option value="1" <?= (int)$user['status'] == 2 ? 'disabled':'' ?> <?= (int)$user['status'] == 3 ? 'disabled':'' ?> <?= (int)$user['status'] == 1 ? 'selected':'' ?> > Đang giao hàng</option>
                            <option value="2" <?= (int)$user['status'] == 3 ? 'disabled':'' ?> <?= (int)$user['status'] == 3 ? 'disabled':'' ?> <?= (int)$user['status'] == 2 ? 'selected':'' ?>>Giao hàng thành công</option>
                            <option value="3" <?= (int)$user['status'] == 3 ? 'selected':'' ?> >Đơn hàng bị hủy</option>
                        </select>
                        <br><br>
