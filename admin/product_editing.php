...
<link rel="icon" type="logo/png" href="images/logo1.png">
<style>
    .danhmuc{
    min-width: 200px;
    height: 30px;
                ...
                if (isset($_POST['name']) && !empty($_POST['name']) && isset($_POST['price']) && !empty($_POST['price'])) {
                    ...
                    if (!isset($error)) {
                        if ($_GET['action'] == 'edit' && !empty($_GET['id'])) { //Cập nhật lại sản phẩm
                            $result = mysqli_query($con, "UPDATE `product` SET `menu_id` = '" . $_POST['menu_id'] . "',`name` = '" . $_POST['name'] . "',`quantity` = '" . $_POST['quantity'] . "',`image` =  '" . $image . "', `price` = " . str_replace('.', '', $_POST['price']) . ", `price_new` = " . str_replace('.', '', $_POST['price_new']) . ", `content` = '" . $_POST['content'] . "', `last_updated` = " . time() . " WHERE `product`.`id` = " . $_GET['id']);
                        } else { //Thêm sản phẩm
                            $result = mysqli_query($con, "INSERT INTO `product` (`id`,`menu_id`, `name`,`quantity`, `image`, `price`,`price_new`, `content`, `created_time`, `last_updated`) VALUES (NULL, '" . $_POST['menu_id'] . "','" . $_POST['name'] . "','" . $_POST['quantity'] . "','" . $image . "', " . str_replace('.', '', $_POST['price']) . "," . str_replace('.', '', $_POST['price_new']) . ", '" . $_POST['content'] . "', " . time() . ", " . time() . ");");
                        }
